package com.project.tshop.catalog.service;
...
public class ProductService {
    ...
    public Page<ProductListItemResponse> list(String search, String category, Integer page, Integer size) {
        ...
        return productRepository.search(normalizeKeyword(search), normalizeKeyword(category), pageable)
    ...
    public ProductDetailResponse getById(UUID id) {
        ...
        List<ProductImageResponse> images = productImageRepository.findResponsesByProductId(id).stream()
                .map(this::resolveImageUrl).toList();
    ...
    @Transactional
    public ProductDetailResponse create(ProductCreateRequest request, MultipartFile thumbnail, List<MultipartFile> images) {
        ...
        if (galleryImages.size() > MAX_GALLERY_IMAGES) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "A product supports up to " + MAX_GALLERY_IMAGES + " gallery images");
        }
        ...
        try {
            ...
            Product product = new Product(null, category, name, slug, request.getPrice(),
                    request.getStockQuantity(), normalizeOptionalText(request.getDescription()),
                    null, thumbnailPath, status, null, new ArrayList<>());

            Product savedProduct = productRepository.save(product);
            List<ProductImage> productImages = new ArrayList<>();

            productImages.add(new ProductImage(null, savedProduct, thumbnailPath, name + " thumbnail", 1, null));
            ...
            for (MultipartFile galleryImage : galleryImages) {
                ...
                productImages.add(new ProductImage(null, savedProduct, imagePath, name + " image " + sortOrder, sortOrder, null));
                sortOrder++;
            }

            productImageRepository.saveAll(productImages);
            ProductDetailResponse detail = productRepository.findDetailBySlug(slug)
                    .orElseThrow(() -> new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to load created product"));

            detail.setThumbnail(minioStorageService.toPublicUrl(detail.getThumbnail()));
            detail.setImages(productImageRepository.findResponsesByProductId(detail.getId()).stream()
                    .map(this::resolveImageUrl).toList());
            return detail;
        } catch (Exception exception) {
            uploadedKeys.forEach(minioStorageService::deleteObject);
            if (exception instanceof ResponseStatusException rse) throw rse;
    ...
    private ProductImageResponse resolveImageUrl(ProductImageResponse response) {
        return new ProductImageResponse(minioStorageService.toPublicUrl(response.getUrl()), response.getAltText(), response.getSortOrder());
    }
    ...
    private void validateRequiredImage(MultipartFile file, String fieldName) {
        if (file == null || file.isEmpty()) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, fieldName + " is required");
        String contentType = file.getContentType();
        if (contentType == null || !contentType.toLowerCase(Locale.ROOT).startsWith("image/")) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, fieldName + " must be an image file");
        }
        try (InputStream inputStream = file.getInputStream()) {
            if (ImageIO.read(inputStream) == null) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, fieldName + " must be a valid image");
    ...
    private String resolveCreateSlug(String requestedSlug, String name) {
        ...
        if (slug.isBlank()) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Slug is invalid");
        if (productRepository.existsBySlug(slug)) throw new ResponseStatusException(HttpStatus.CONFLICT, "Product slug already exists");
        return slug;
    }
    ...
    private String uploadAndTrack(MultipartFile file, String slug, String keyHint, List<String> uploadedKeys) {
    ...
    public List<ProductDetailResponse> batchGetDetails(List<UUID> ids) {
        if (ids == null || ids.isEmpty()) return List.of();
        List<ProductDetailResponse> details = productRepository.findDetailsByIds(ids);
        details.forEach(detail -> detail.setThumbnail(minioStorageService.toPublicUrl(detail.getThumbnail())));
        return details;
    }
    ...
    public void deductStock(Map<UUID, Integer> updates) {
        updates.forEach((id, qty) -> {
            int updated = productRepository.deductStock(id, qty);
            if (updated == 0) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Insufficient stock for product " + id);
            }
        });
    }
    ...
    public void restoreStock(Map<UUID, Integer> updates) {
        updates.forEach(productRepository::restoreStock);
    }
}

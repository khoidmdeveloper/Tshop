...
public class CategoryService {
    ...
    public CategoryResponse create(CategoryCreateRequest request) {
        ...
        Category category = new Category(null, name, slug, parent, new ArrayList<>(), new ArrayList<>());
        categoryRepository.save(category);
        return categoryRepository.findResponseBySlug(slug)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to load created category"));
    }
    ...
    private NameResolution resolveUpdateName(CategoryResponse existing, CategoryUpdateRequest request) {
        ...
        if (requested == null || requested.isBlank()) return new NameResolution(existing.getName(), false);
    ...
    private String resolveUpdateSlug(CategoryUpdateRequest request, String name, boolean nameChanged, UUID currentId) {
        ...
        if (nameChanged) return resolveSlug(null, name, currentId);
    ...
    private Category resolveParent(UUID parentId, UUID currentId) {
        ...
        if (parentId.equals(currentId)) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Parent category cannot be itself");
        if (!categoryRepository.existsById(parentId)) throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Parent category not found");
    ...
    private String resolveSlug(String requestedSlug, String fallbackName, UUID currentId) {
        ...
        if (base == null || base.isBlank()) base = fallbackName;
    ...
    private void ensureSlugUnique(String slug, UUID currentId) {
        if (categoryRepository.existsBySlug(slug)) {
            if (currentId == null) throw new ResponseStatusException(HttpStatus.CONFLICT, "Slug already exists");
            ...
            if (!existing.getId().equals(currentId)) throw new ResponseStatusException(HttpStatus.CONFLICT, "Slug already exists");
        }
    }
